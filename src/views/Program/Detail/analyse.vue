<template>
  <div class="container mx-auto p-4">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-6">
      <div class="p-6 shadow-lg rounded-lg hover:shadow-2xl transition duration-300 ease-in-out">
      ...
      <div class="p-6 shadow-lg rounded-lg hover:shadow-2xl transition duration-300 ease-in-out">
        <h3 class="text-xl font-semibold text-gray-800 mb-3">平均页面停留时长</h3>
        <p class="text-3xl font-bold text-yellow-600">{{ pageDuration.toFixed(2) }} 秒</p>
      </div>
    </div>
    ...
    <div class="mb-6">
      ...
      <div v-show="activeCollapse === 'location'" class="mt-4 p-4 bg-gray-50 rounded-md shadow-sm">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div v-for="(location, country) in sortedLocationInfo" :key="country" class="p-3 bg-white rounded-md shadow-md">
            <p class="font-medium text-gray-700">{{ location.country }}:</p>
            <p class="text-sm text-gray-500">{{ location.count }} 次浏览 ({{ (location.ratio * 100).toFixed(2) }}%)</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="mb-6">
      ...
      <div v-show="activeCollapse === 'referrer'" class="mt-4 p-4 bg-gray-50 rounded-md shadow-sm">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div v-for="(referrer, index) in sortedReferrerInfo" :key="index" class="p-3 bg-white rounded-md shadow-md">
            <p class="font-medium text-gray-700">{{ referrer.Domain || "直接访问" }}:</p>
            <p class="text-sm text-gray-500">{{ referrer.Count }} 次浏览 ({{ (referrer.Ratio * 100).toFixed(2) }}%)</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="mb-6">
      ...
      <div v-show="activeCollapse === 'page'" class="mt-4 p-6 bg-gray-50 rounded-md shadow-sm">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div v-for="(page, path) in sortedPageInfo" :key="path" class="p-4 bg-white rounded-md shadow-md">
            <p class="font-medium text-gray-700">
              <a :href="websiteURL + path" target="_blank" class="text-blue-500 hover:underline">
                {{ decodeURIComponent(websiteURL + path) }}
              </a>
            </p>
            <p class="text-sm text-gray-500">{{ page.count }} 次浏览 ({{ (page.ratio * 100).toFixed(2) }}%)</p>
          </div>
        </div>
      </div>
    </div>
...
// 计算排序后的地理位置信息，转换为数组并进行排序
const sortedLocationInfo = computed(() => {
  return Object.entries(locationInfo.value)
    .map(([key, value]) => ({ country: key, ...value }))
    .sort((a, b) => b.count - a.count);
});

// 计算排序后的来源信息
const sortedReferrerInfo = computed(() => {
  return referrerInfo.value.sort((a, b) => b.Count - a.Count);
});

// 计算排序后的页面信息
const sortedPageInfo = computed(() => {
  return Object.entries(pageInfo.value)
    .sort(([, a], [, b]) => b.count - a.count)
